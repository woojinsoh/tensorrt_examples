...
from utils.module_wrappers import T5TRTEncoderModule, T5TRTDecoderModule, T5TRTConditionalGenerationWrapper
...
def main():
  ...
  dec_input_ids = np.ones((batch_size, 1), np.int32) * config.decoder_start_token_id

  with torch.no_grad():
      # PyTorch Inference
      th_enc_out = th_model.encoder(input_ids = torch.tensor(input_ids).to(device), attention_mask = torch.tensor(attention_mask).to(device))
      th_dec_out = th_model.decoder(input_ids = torch.tensor(dec_input_ids).to(device), encoder_hidden_states = th_enc_out.last_hidden_state, encoder_attention_mask = torch.tensor(attention_mask).to(device))
      th_head_out = th_model.lm_head(th_dec_out.last_hidden_state * config.d_model ** -0.5)
      th_generate_out = th_model.generate(input_ids = torch.tensor(input_ids).to(device))
      ...
      # TRT Inference
      trt_encoder = T5TRTEncoderModule(TRT_ENCODER_FILE_PATH, config, max_batch_size=batch_size)
      trt_decoder = T5TRTDecoderModule(TRT_DECODER_FILE_PATH, config, max_batch_size=batch_size, max_output_seq=256)
    
      trt_enc_out = trt_encoder(input_ids, attention_mask)
      trt_dec_out = trt_decoder(dec_input_ids, trt_enc_out, attention_mask)
      trt_head_out = th_model.lm_head(torch.tensor(trt_dec_out * config.d_model ** -0.5).to(device))


  # Check the accuracy difference
  print("[The value difference between PyTorch and TRT inference]")
  enc_diff = abs(torch.tensor(trt_enc_out) - th_enc_out.last_hidden_state.cpu())
  dec_diff = abs(torch.tensor(trt_dec_out) - th_dec_out.last_hidden_state.cpu())
  head_diff = abs(trt_head_out.cpu() - th_head_out.cpu())
  print_diff_percentile(enc_diff, "encoder_diff")
  print_diff_percentile(dec_diff, "decoder_diff")
  print_diff_percentile(head_diff, "lm_head_diff")
